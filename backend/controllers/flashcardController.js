...
// Run Python flashcard generator
    ...
    return new Promise((resolve, reject) => {
        ...
        pythonProcess.on("close", (code) => {
            if (code !== 0) {
                return reject(
                    new Error(
                        stderr || "Python generator exited with an error."
                    )
                );
            }
            ...
            try {
                ...
                reject(
                    new Error(
                        `Invalid JSON from Python generator: ${error.message}`
                    )
                );
            }
        });
...
const saveFlashcards = async (req, res) => {
    try {
        ...
        const savedFlashcards =
            await Flashcard.insertMany(documents);
